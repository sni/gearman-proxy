...
# run check_gearman with perf data filter
{
    my $res = `/omd/sites/demo/lib/monitoring-plugins/check_gearman -H 127.0.0.1:4730 -q proxy_status -s 'check perfdatafilter=^(server|queues|total_|bytes_)'`;
    is($?, 0, "check_gearman worked");
    like($res, "/OK - proxy version/", "result contains OK");
    like($res, "/'queues'=3/", "result contains queues");
    like($res, "/'server'=2/", "result contains server");
    unlike($res, "/'hostgroup_worker1::jobs'=[0-9]+c/", "result contains not hostgroup_worker1::jobs");
};
